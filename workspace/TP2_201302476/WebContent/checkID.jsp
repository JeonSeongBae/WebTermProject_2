...
<body>
	<%
		...
		if (matcher.find()) { // ID 형식을 확인함
			answer = "POSSIBLE";
			String filePath = "C:\\"; // 인증받지 않은 IDData들이 들어있는 폴더
			...
			filePath = "C:\\WP\\"; // 인증받지 않은 IDData들이 들어있는 폴더
			file = new File(filePath); // 파일경로에 파일 객체를 생성
			files = file.listFiles(); // 파일리스트들을 불러옴
			for (int i = 0; i < files.length; i++) {
				if (files[i].getName().equals(ID + ".txt")) { // 중복된 아이디가 있는지 확인
					answer = "DUPLICATION";
					break;
